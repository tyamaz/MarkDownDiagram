var mdg_draw = function(_base) {
    ...
    this.create = function(editable,box) {
        ...
        var e = $(type).addClass("box").attr('id',box.id).attr('title',box.id).attr('draggable',d).html(inner);
        if(box.cls){
            if(typeof box.cls == "string"){ box.cls = [box.cls]; }
            for(var i in box.cls){
                e.addClass(box.cls[i]);
            }
        }
        this.base.append(e);
        lp = {
            x: parseInt(pos.x) + round(parseInt(e.css('width')) / this.em) + 2,
            y: parseInt(pos.y) + 1
        };
        return pos;
    }
            ...
            for(var i in b.bl){
                ...
                if(m_sep.exec(cl)){
                    ...
                    conn.push({
                        ...
                        to: a[6],
                        param: {
                            s_pos: (fp + (l.length + ((b.title!=null) ? 1 : 0))),
                            e_pos: tp,
                            cls: a[3],
                            arrow: ar,
                            type:a [7]
